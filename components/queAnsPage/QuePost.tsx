      ...
      <div className="px-6 md:py-6 py-4 flex justify-between">
        ...
        <div className="w-0 flex-1">
          <div className="flex justify-between max-h-60 mt-1 space-x-2 text-xs">
            ...
            <span className="mt-3 font-style-5-username dark:text-white">{isAnonymous ? ('Anonymous') : <Link className=" hover:underline cursor-pointer" href={`/profile/${post.uid}`}>{post.name}</Link>}</span>{" "}
            ...
            <div className="font-style-5-username mt-[11px]">{dateString}</div>

            {/* {formatTimeToNow(new Date(post.createdAt))} */}
          </div>
          {/* <a href={`/postPage/${post.id}`}> */}
            <h1 className="font-style-2 font-bold py-3 leading-6 dark:text-white">
              {post.title}
            </h1>
          {/* </a> */}
